...
m.Add(sum(x[(c,s,"ליאור")] for c in CLASSES for s in NONFRI if (c,s,"ליאור") in x)<=8)
# מרים מוגבלת למכסה 23 פחות 10 שעות חטיבה
m.Add(sum(x[(c,s,"מרים")] for c in CLASSES for s in NONFRI if (c,s,"מרים") in x)<=13)
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_h.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_h.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_h.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
