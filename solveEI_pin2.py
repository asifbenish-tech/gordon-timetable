...
# מרים: מקסימום 2 שעות ביום ג ו-2 בחמישי ביסודי (משאיר לה זמן לחטיבה)
for d in (2,4):
    dv=[x[(c,(d,h),"מרים")] for c in CLASSES for h in range(1,DAY_HOURS[d]+1) if (c,(d,h),"מרים") in x]
    if dv: m.Add(sum(dv)<=2)
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sed_M.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("co_zofia6.json","w",encoding="utf-8").write(json.dumps(
        {tag+"|"+f"{sl[0]},{sl[1]}":1 for (tag,sl),b in co.items() if sol.Value(b)},ensure_ascii=False))
    io.open("sol_M.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    ...
    io.open("dev_M.txt","w",encoding="utf-8").write("\n".join(dev))
