...
POOL={"טלי":(3,B_CL),"ליאור":(4,CLASSES),"מרים":(8,CLASSES),"צופיה":(4,G_CL),"שחר":(2,[c for c in CLASSES if c[0] in "אבג"])}
...
# שעות מקבילות של צופיה (מצטרפת, לא מחליפה)
co={}
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sed_H.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("co_zofia.json","w",encoding="utf-8").write(json.dumps(
        {tag+"|"+f"{sl[0]},{sl[1]}":1 for (tag,sl),b in co.items() if sol.Value(b)},ensure_ascii=False))
    io.open("sol_H.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    ...
    io.open("dev_H.txt","w",encoding="utf-8").write("\n".join(dev))
