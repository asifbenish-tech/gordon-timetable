...
erz={d:[x[(c,(d,h),"אנגלית","ארז")] for c in HCLASSES for h in range(1,HDAY[d]+1)
        if (c,(d,h),"אנגלית","ארז") in x] for d in (2,3)}
m.Add(sum(erz[2])+sum(erz[3])==8)
m.Add(sum(erz[2])>=2); m.Add(sum(erz[3])>=2)
...
for c in HCLASSES:
    for subj in NEED:
        ...
        for d in range(6):
            ...
            if v: m.Add(sum(v)<= (3 if (subj=="אנגלית" and GRADE[c]=="ז" and d==3) else 2))
...
m.Minimize(400*sum(vend.values())+5*sum(late) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
