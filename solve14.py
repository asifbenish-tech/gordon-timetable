...
for t in CIRCLE_TEACHERS:
    ...
    if f=="ג": m.Add(v==0)
    if f=="ב": m.Add(v==1)
ORIG_MON=["דני","אינס","דניאל","תמיר","אלי","נעמי","תניה","דליה","אנה"]
for _t in gmon: m.Add(gmon[_t]==(1 if _t in ORIG_MON else 0))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_k.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_k.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_k.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
