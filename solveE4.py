...
for c in CLASSES:
    for t in allowed[c]:
        ...
        for h in range(1,DAY_HOURS[2]+1):
            if t in CIR_TUE and (c,(2,h),t) in x: m.Add(x[(c,(2,h),t)]+blkh[("u",h)]<=1)
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sed_new.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("sol_elem5.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    ...
    io.open("dev_elem5.txt","w",encoding="utf-8").write("\n".join(dev))
