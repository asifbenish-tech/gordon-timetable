...
for c in HCLASSES:
    for subj in NEED:
        ...
        for d in range(6):
            ...
            if v: m.Add(sum(v)<= (3 if (subj=="מתמטיקה" and GRADE[c]=="ז") else 2))
...
m.Minimize(5*sum(late) - 50*sum(hadar) - 200*sum(own) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sol_hat_a.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    peo={f"{DAY_NAMES[d]} ש{h}":g for (g,d,h) in pe if sol.Value(pe[(g,d,h)])}
    io.open("duty_a.json","w",encoding="utf-8").write(json.dumps(
        {c:DAY_NAMES[[d for d in range(5) if sol.Value(duty[(c,d)])][0]] for c in HCLASSES},
        ensure_ascii=False,indent=1))
    io.open("pe_hat_a.json","w",encoding="utf-8").write(json.dumps(peo,ensure_ascii=False,indent=1))
