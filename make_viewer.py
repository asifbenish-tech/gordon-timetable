...
for c in HCLASSES:
    for (d,h) in HSLOTS:
        ...
        if not v: continue
        subj,t=(v.split(" – ")+[""])[:2]
        if t and t not in ("מגמות","חסר מורה","שכבת ט יחד"):   # שירה בציבור/גיבוש כן נספרים למי שמלווה בפועל
            if t=="שרית + חסן":
...
add_t("פנינה","סדירות",0,3,"הדרכה")   # הערה בלבד במערכת האישית - ממשיכה ללמד בכיתתה
...
_cmt={}
_KINDS=("הדרכ","מפגשה","ישיב","אסיפ","פגיש")   # "פגיש" - פגישות הנהלה ופגישות אישיות
...
from data2 import TEACH_DESC as _TEACH_SUBJ   # תיאור 'מה מלמד/ת' - מקור אחד ב-data2
