...
def away_map(hr):
    ...
    for day in ("שני","שלישי"):
        if hr in SED.get("קבוצת "+day,[]):
            for h in SED["מעגלי שיח "+day]: out[f"{CM[day]},{h}"]="מעגל שיח מחנכים"
    if hr in NIHUL:
        for h in SED["ישיבת ניהול שלישי"]: out[f"2,{h}"]="ישיבת ניהול"
    return out
...
for c in CLASSES:
    ...
    for (d,h) in SLOTS:
        ...
        elif not t: cell["t"]="מנהל"; cell["s"]="זמני – עד סגירת החוסר"; cell["k"]="hole"
...
QUOTA_FILE={"יעל":12,"חגית":8,"דליה":24,"תמיר":24,"סימה":24,"לייה":23,"פנינה":23,"מירי":23,
 ...
 "ארז":8,"הדר":8,"ליאור":10,"אינס":24,"שחר":10,"אנה":25,"אביטל":25,"מרים":23,"שיר":4,
 "דניאל":26,"צופיה":18,"רובי":6,"חסן":26,"דני":24,"טלי":10,"שרית":28,"יערה":25,
...
_CM={"שני":1,"שלישי":2}
_QF={"מרים":23,"צופיה":18,"טלי":10,"ליאור":10,"שחר":10,"דניאל":26,"דני":24,"אינס":24,
 "מירי":23,"אנה":25,"פנינה":23,"אביטל":25,"יערה":25,"לייה":23,"דליה":24,"תניה":18,
 "אורנה":18,"סימה":24,"פאני":16,"חסן":26,"שרית":28}
_load={}
for _c in CLASSES:
    for (_d,_h) in SLOTS:
        _t=S[_c][f"{_d},{_h}"]
...
SOLUTIONS=[
 ...
 {"t":"להעלות את ליאור ל-10 שעות בפועל","d":"אישרת להעלות אותה מ-8 ל-10. יש לה מקום פנוי במכסה.","i":"נמוכה"},
...
SYS_CORE=[
 ...
 ("מעגלי שיח מחנכים - שני 3-4","תדהר+עדי עם: אנה, אינס, אסיף, דליה, דני, דניאל, מירי, נעמי, תמיר."),
 ("מעגלי שיח מחנכים - שלישי 5-6","תדהר+גלית עם: אביטל, אורנה, אלי, גלית, יערה, לייה, פנינה, שרית, תניה."),
 ("ישיבת ניהול","שלישי 3-4: לייה, שרית, יערה, צופיה, אסיף, אלי."),
 ...
 ("חוסרים ביסודי","במשבצות ללא מורה המנהל נכנס בינתיים - שיבוץ זמני עד לסגירת החוסר."),
]
...
_TEACH_SUBJ={
 "נעמי":"מחנכת ז נעמי · שפה ז+ח · ספרות ז+ח+ט","אלי":"מנהל החטיבה · מחנך ז אלי · היסטוריה ז+ח+ו · גיאוגרפיה ח · מגמת יזמות",
 "גלית":"מחנכת ח · אנגלית ח+ט · שיעורים עם ארז בז","תמיר":"מחנך ט תמיר · תנ\"ך ז+ט · היסטוריה ט · אזרחות ט · ליווי מגמות חמישי",
 "אסיף":"מחנך ט אסיף · שפה ט · מדעים ז · חינוך · מגמת יזמות","מרים":"ערבית ז+ח+ט · סטאז עברית ביסודי (13 שעות, דגש א-ג)",
 ...
 "ליאור":"יסודי (עד 3 ימים)","שחר":"יסודי (שני-רביעי, לא בשעה אחרונה)","דניאל":"מחנך ג דניאל","דני":"מחנך ה דני (20 שעות)",
