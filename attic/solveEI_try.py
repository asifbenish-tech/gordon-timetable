...
POOL={"טלי":(3,B_CL),"ליאור":(6,CLASSES),"מרים":(10,CLASSES),"צופיה":(8,[c for c in CLASSES if c[0] in "אבג"]),"שחר":(2,[c for c in CLASSES if c[0] in "אבג"])}
...
CIR_MON=["דני","אינס","דניאל","תמיר","אלי","נעמי","תניה","דליה","גלית"]
CIR_TUE=["אסיף","פנינה","יערה","אורנה","אנה","שרית","לייה","מירי","אביטל"]
...
for c in ("ג לייה","ג דליה","ג דניאל"):
...
co={}
for tag,cls,hr,n in (("anna","א אנה","אנה",4),("pnina","א פנינה","פנינה",2)):
    vs=[]
    for sl in NONFRI:
...
for t in ("אסיף","תמיר"):
    for c in CLASSES:
        for s2 in NONFRI:
            if s2[0] not in (0,1) and (c,s2,t) in x:
                m.Add(x[(c,s2,t)]==0)

# מרים: אפס בשלישי (חוסם קשה), עד 2 בחמישי
dv=[x[(c,(2,h),"מרים")] for c in CLASSES for h in range(1,DAY_HOURS[2]+1) if (c,(2,h),"מרים") in x]
if dv: m.Add(sum(dv)==0)
dv2=[x[(c,(4,h),"מרים")] for c in CLASSES for h in range(1,DAY_HOURS[4]+1) if (c,(4,h),"מרים") in x]
if dv2: m.Add(sum(dv2)<=2)
...
for t in ("אסיף","תמיר"):
    for c in CLASSES:
        v=[x[(c,s,t)] for s in NONFRI if (c,s,t) in x]
        if v:
            if c.startswith("ו "): m.Add(sum(v)>=1); m.Add(sum(v)<=2)
            else: m.Add(sum(v)==0)
...
m.Minimize(-300*sum(orna)+10000*sum(empty.values())+sum(pen)+200*(len(farm)-sum(farm))+400*(len(keepg)-sum(keepg)))
sol=cp_model.CpSolver(); sol.parameters.max_time_in_seconds=900; sol.parameters.num_workers=8
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sed_N.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("co_zofia7.json","w",encoding="utf-8").write(json.dumps(
        {tag+"|"+f"{sl[0]},{sl[1]}":1 for (tag,sl),b in co.items() if sol.Value(b)},ensure_ascii=False))
    io.open("sol_N.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    ...
    io.open("dev_N.txt","w",encoding="utf-8").write("\n".join(dev))
