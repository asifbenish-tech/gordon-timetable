...
DAYS_OFF.setdefault("מאמי",[]); DAYS_OFF.setdefault("רובי",["שני","חמישי","שישי"])
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_b.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    # deviation report
    lines=[]
    for c in CLASSES:
    ...
    io.open("deviations_b.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
