...
m.Minimize(5000*sum(vend.values())+5*sum(late) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("d7.txt","w",encoding="utf-8").write(chr(10).join(
        f"END-VIOL {c} {DAY_NAMES[d]} ש{h}" for (c,d,h),v in sorted(vend.items()) if sol.Value(v)) or "none")
