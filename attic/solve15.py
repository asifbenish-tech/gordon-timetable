...
POOL={"טלי":(3,B_CL),"ליאור":(0,CLASSES),"מרים":(6,DEU),"חסן":(0,DEU),"צופיה":(4,G_CL)}
...
for t in CIRCLE_TEACHERS:
    ...
    if f=="ג": m.Add(v==0)
    if f=="ב": m.Add(v==1)
LOCK_MON=["אינס","אלי","אנה","דני","דניאל","לייה","נעמי","תמיר","תניה"]
for _t in gmon: m.Add(gmon[_t]==(1 if _t in LOCK_MON else 0))
...
m.Add(blk_h[("m",3)]==1)   # מעגלי שיח שני = שעות 3-4 (10:00-11:30, כמקור)
m.Add(blk_h[("u",1)]==1)   # מעגלי שיח שלישי = שעות 1-2 (8:10-9:45)
for t in CIRCLE_TEACHERS:
...
# hard cap: ליאור = 8 שעות בסך הכל

# מרים מוגבלת למכסה 23 פחות 10 שעות חטיבה

# keep the approved א-ג schedule unless something forces a change
AG=[c for c in CLASSES if c[0] in "אבג"]
appr=json.load(io.open("approved_ag.json",encoding="utf-8"))
keep=[]
for c in AG:
    for s in NONFRI:
        t=appr[c][f"{s[0]},{s[1]}"]
        if (c,s,t) in x: keep.append(x[(c,s,t)])
# ---- מכסה פרונטלית לכל מורה (מקובץ המורים), פחות מחויבויות חטיבה/מגמה ----
MAXQ={"אנה":25,"אסיף":20,"דליה":24,"לייה":23,"פנינה":23,"שרית":28,"תניה":18,
      "יערה":25,"דני":24,"אורנה":18,"צופיה":23,"פאני":16,"תמיר":24,"מירי":23,
      "אינס":24,"אביטל":25,"דניאל":26,"מרים":23,"סימה":24,"חסן":26,"ליאור":8,
      "טלי":10,"שחר":12}
HATIVA={"לייה":2,"שרית":6,"חסן":10,"מרים":10,"אסיף":18,"תמיר":22}   # שעות מחוץ ליסודי
capviol=[]
for _t,_q in MAXQ.items():
    _lim=_q-HATIVA.get(_t,0)
    _v=[x[(c,s,_t)] for c in CLASSES for s in NONFRI if (c,s,_t) in x]
    _fx=sum(1 for (c,s),tt in fixed.items() if tt==_t)
    if _v:
        ex=m.NewIntVar(0,20,f"cap_{_t}")
        m.Add(sum(_v)-ex<=_lim-_fx); capviol.append((_t,ex))
        pen.append(3000*ex)

m.Minimize(10000*sum(empty.values())+500*(len(keep)-sum(keep))+sum(pen))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_m.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_m.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_m.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
    print("deviations:",len(lines))
    cv=[f"{t}: חריגה של {sol.Value(v)} ש' מעל המכסה" for t,v in capviol if sol.Value(v)]
    io.open("capviol.txt","w",encoding="utf-8").write(chr(10).join(cv) or "אין חריגות מכסה")
