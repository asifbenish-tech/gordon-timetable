...
_SECT["cur"] = "---- חטיבה: ספורט שכבתי, 3 שעות רצופות בראשון "
pe={}
...
_SECT["cur"] = "שרית/חסן חסומים בשעות הספורט של החטיבה"
for c in CLASSES:
    for t in ("שרית","חסן"):
...
_SECT["cur"] = "צופיה: יום חופש קשיח בחמישי (בהתאם לבקשתה) + ש"
for d in (4,):
    for h in range(1,DAY_HOURS[d]+1):
        for c in CLASSES:
...
_SECT["cur"] = "חסן ופאני מורי ספורט: מכסה לכל כיתה קשיחה (שעה"
for t in ("חסן","פאני"):
    for c in CLASSES:
...
_SECT["cur"] = "מגבלות ימי עבודה לפי הקובץ: ליאור 3, שחר 3, טל"
...
_SECT["cur"] = "שחר: לא בשעה האחרונה של כל יום (מועדונית)"
for d in range(5):
    for c in CLASSES:
...
_SECT["cur"] = "שחר: לא לבוא ליום עם שעה אחת בלבד (0 או 2+)"
for d in range(5):
...
_SECT["cur"] = "---- סדירויות שבועיות: מעגלי שיח (חלוקה מקורית"
...
# ---- תיקונים לפי המשוב ----
_SECT["cur"] = "---- תיקונים לפי המשוב ----"
# דני: 11 בכיתתו + 9 אצל תניה = 20 בדיוק
_SECT["cur"] = "דני: 11 בכיתתו + 9 אצל תניה = 20 בדיוק"
...
_SECT["cur"] = "דניאל: לפחות 17 בכיתה שלו, ולא בכיתות ו"
...
_SECT["cur"] = "צופיה: 2 שעות אנגלית בכל כיתת ג, וסה'כ עד 18"
...
_SECT["cur"] = "צופיה: יום חופש חמישי"
# שעות מקבילות של צופיה (מצטרפת, לא מחליפה) - גם הן לא בחמישי
_SECT["cur"] = "שעות מקבילות של צופיה (מצטרפת, לא מחליפה) - גם"
co={}
...
_SECT["cur"] = "טלי: יומיים עבודה בלבד"
tdays=[]
for d in range(5):
...
_SECT["cur"] = "אורנה: כמה שיותר בכיתה שלה"
...
_SECT["cur"] = "נעילת שעות אסיף/תמיר ביסודי לימים ראשון/שני בל"
for c in CLASSES:
    for s2 in NONFRI:
...
_SECT["cur"] = "מרים סטאז עברית (miriam_staj): כל 15 שעות היסו"
...
_SECT["cur"] = "מרים: אפס יסודי בשלישי (שמורה לחטיבה), מקס 4 ב"
for d in range(5):
...
_SECT["cur"] = "הילית ויפעת בשכבת א: שעה אחת כל אחת - מטופל בנ"
...
_SECT["cur"] = "ספורט יסודי (פאני+חסן): שיעור אחד ליום לכל היו"
for c in CLASSES:
    for d in range(5):
...
_SECT["cur"] = "אינס: 2 מדעים ברצף - כל יום חוץ משלישי (יום הח"
...
_SECT["cur"] = "אסיף/תמיר: שעה בכל כיתת ו"
for t in ("אסיף","תמיר"):
    for c in CLASSES:
        v=[x[(c,s,t)] for s in NONFRI if (c,s,t) in x]
        if v:
...
_SECT["cur"] = "חווה חקלאית: מחנכות ג' עם הכיתה בשני ש1-2 (רך)"
farm=[]
...
_SECT["cur"] = "---- נעילת סדירויות לפי sed_J.json (החלטות מאו"
...
_SECT["cur"] = "שלישי: רק שני שילובים אפשריים (מגמות תופסות 1-"
for _h in range(1,7):
...
_SECT["cur"] = "================= צד החטיבה (בתוך אותו מודל) ="
# -*- coding: utf-8 -*-
_SECT["cur"] = "-*- coding: utf-8 -*-"
import io, json, collections
from ortools.sat.python import cp_model
from hdata import *
...
_SECT["cur"] = "מודל משותף (מוגדר בצד היסודי)"
# ---- חינוך גופני שכבתי: ראשון ש1-3 ורביעי ש4-6, שעה לכל שכבה ----
_SECT["cur"] = "---- חינוך גופני שכבתי: ראשון ש1-3 ורביעי ש4-6"
hpe={}
...
for c in HCLASSES:
    ...
    for subj,per in NEED.items():
        ...
        if subj=="חינוך":
            ...
            if c=="ט אסיף":     _sing="תמיר"          # שיעור כפול: תמיר עם שתי כיתות ט
...
_SECT["cur"] = "MISSING_FRI: ט אסיף בשישי - אסיף בחופש ואין מו"
_MC="ט אסיף"
for _c9 in HCLASSES:
...
_SECT["cur"] = "חינוך גופני: לפי הבלוקים השכבתיים"
for c in HCLASSES:
    g=GRADE[c]
    for s in HSLOTS:
...
_SECT["cur"] = "אופיר: שעה אחת בכל כיתת ז, יום שלישי בלבד"
for c in HCLASSES:
    for s2 in HSLOTS:
...
_SECT["cur"] = "מורה חיצוני מתמטיקה: עד 3 ימי עבודה, לא ביום ש"
ext_days=[]
for _d in range(6):
...
_SECT["cur"] = "ארז: רביעי + חמישי, בכל יום 2 שעות בכל כיתת ז "
...
_SECT["cur"] = "שיר: מחליפה את אלי עם כיתתו בשישי (מגיעה רק בי"
...
_SECT["cur"] = "גלית: שני שיעורים משותפים עם ארז בכיתות ז (נספ"
gj={}
...
_SECT["cur"] = "אלי: נוכחות מוגברת בכיתה שלו - תנ'ך ורב מלל בז"
...
_SECT["cur"] = "מורה אחד בכל רגע (כולל מול היסודי)"
for s in HSLOTS:
...
_SECT["cur"] = "מתמטיקה ז: הדר 8 שעות (4+4 לפי הסדין), צבי משל"
...
_SECT["cur"] = "הדר: בדיוק יומיים, בכל יום 2 שעות בכל כיתת ז ("
# הדר מפוצלת: שעה בכל כיתת ז בשלישי 5-6, והיתרה ביומיים (2-3 ימי עבודה)
_SECT["cur"] = "הדר מפוצלת: שעה בכל כיתת ז בשלישי 5-6, והיתרה "
for _c in [c for c in HCLASSES if GRADE[c]=="ז"]:
...
_SECT["cur"] = "מניעת חלונות במערכת של תמיר: השעות שלו רצופות "
for _t in ("תמיר",):
    for _d in range(6):
        _busy={}
        for _h in range(1,HDAY[_d]+1):
...
_SECT["cur"] = "'חסר מורה': רק בט אסיף, רק בשישי, בדיוק 4 שעות"
# ט אסיף בשישי: אסיף בחופש -> בדיוק 4 שעות חסרות
_SECT["cur"] = "ט אסיף בשישי: אסיף בחופש -> בדיוק 4 שעות חסרות"
# בט אסיף חסר מורה נרשם כחינוך בלבד (miss_hinuch)
_SECT["cur"] = "בט אסיף חסר מורה נרשם כחינוך בלבד (miss_hinuch"
for _k in [k for k in hx if k[0]==_MC and k[3]=="חסר מורה" and k[2]!="חינוך"]:
    m.Add(hx[_k]==0)
# אנגלית=גלית ושפה=נעמי/אסיף - חסר מורה אסור בהן (no_miss_eng)
_SECT["cur"] = "אנגלית=גלית ושפה=נעמי/אסיף - חסר מורה אסור בהן"
...
if _miss_fri: m.Add(sum(_miss_fri)==2)   # תמיר מכסה 2 (שירה + כפול)
...
_SECT["cur"] = "שלישי: כל הכיתות עד שעה 6 - מותר 'חסר מורה' בק"
...
_SECT["cur"] = "יום המגמות: חינוך בש5, ואז הביתה"
for _cz in ("ז נעמי","ז אלי","ח גלית"):
    _v5=[hx[k] for k in hx if k[0]==_cz and k[1]==(2,5) and k[2]=="חינוך"]
    if _v5: m.Add(sum(_v5)==1)
for _c9p in T9:
    _v5=[hx[k] for k in hx if k[0]==_c9p and k[1]==(4,5) and k[2]=="חינוך"]
    if _v5: m.Add(sum(_v5)==1)
    for _hb in (6,7): m.Add(hfree[(_c9p,(4,_hb))]==1)
# ספרות והיסטוריה: נעמי ואלי בעצמם לפחות שעה בכל כיתה (שיר משלימה בשישי)
_SECT["cur"] = "ספרות והיסטוריה: נעמי ואלי בעצמם לפחות שעה בכל"
for _cz2 in ("ז נעמי","ז אלי","ח גלית"):
    _vv=[hx[(_cz2,s2,"ספרות","נעמי")] for s2 in HSLOTS if (_cz2,s2,"ספרות","נעמי") in hx]
    if _vv: m.Add(sum(_vv)>=1)
    _vv=[hx[(_cz2,s2,"היסטוריה","אלי")] for s2 in HSLOTS if (_cz2,s2,"היסטוריה","אלי") in hx]
    if _vv: m.Add(sum(_vv)>=1)

# שירה בציבור: יום שישי שעה 2, כל החטיבה יחד באולם חדר האוכל
_SECT["cur"] = "שירה בציבור: יום שישי שעה 2, כל החטיבה יחד באו"
for c in HCLASSES:
    for s2 in HSLOTS:
        _sg = "שיר" if HHOME[c]=="אלי" else ("תמיר" if c=="ט אסיף" else HHOME[c])
        k=(c,s2,"שירה בציבור",_sg)
        if k in hx: m.Add(hx[k]==(1 if s2==(5,2) else 0))

# יום שישי: עדיף מחנך הכיתה, אחרת המחנך המקביל
_SECT["cur"] = "יום שישי: עדיף מחנך הכיתה, אחרת המחנך המקביל"
...
_SECT["cur"] = "סידור חדר אוכל: המחנך/ת עם הכיתה בשעה 5, פעם ב"
duty={}
for c in HCLASSES:
    for d in range(5):
...
_SECT["cur"] = "אסיף מלמד בשישי עם הכיתה שלו (לפחות שעתיים)"
...
_SECT["cur"] = "אסיף: לא מלמד שפה בשעה האחרונה של היום (שעה 6 "
for c in HCLASSES:
    for d in range(6):
...
_SECT["cur"] = "==== חלוקת רב מלל ותנ'ך בכיתות ט (rules_rm) =="
# אסיף: שעת החינוך השנייה שלו בשעה שישית
_SECT["cur"] = "אסיף: שעת החינוך השנייה שלו בשעה שישית"
_a6=[hx[("ט אסיף",(d,6),"חינוך","אסיף")] for d in (1,2,3,4)
     if ("ט אסיף",(d,6),"חינוך","אסיף") in hx]
if _a6: m.Add(sum(_a6)>=1)
_v=[hx[("ט תמיר",s2,"רב מלל","תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,"רב מלל","תמיר") in hx]
if _v: m.Add(sum(_v)==3)
_v=[hx[("ט תמיר",s2,'תנ"ך',"תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,'תנ"ך',"תמיר") in hx]
if _v: m.Add(sum(_v)==2)
for _c9 in T9:
    _v=[hx[(_c9,s2,"רב מלל","נעמי")] for s2 in HSLOTS if (_c9,s2,"רב מלל","נעמי") in hx]
    if _v: m.Add(sum(_v)==1)

# ח גלית: שעת החינוך הנוספת = שעה שביעית עם גלית (galit_h7)
_SECT["cur"] = "ח גלית: שעת החינוך הנוספת = שעה שביעית עם גלית"
_g7=[hx[("ח גלית",(d,7),"חינוך","גלית")] for d in (1,3)
     if ("ח גלית",(d,7),"חינוך","גלית") in hx]
if _g7: m.Add(sum(_g7)==1)

# ח גלית: מינימום 5 שעות לימוד בכל יום א-ה (hg_min)
_SECT["cur"] = "ח גלית: מינימום 5 שעות לימוד בכל יום א-ה (hg_m"
# אלי לא מלמד בשכבת ט כלל (no_eli_9); תנ"ך ט אסיף = תמיר
_SECT["cur"] = "אלי לא מלמד בשכבת ט כלל (no_eli_9); תנ'ך ט אסי"
...
_SECT["cur"] = "מאמי: מלמדת רק חמישי 5-6, עד שעתיים (mami_thu)"
...
_SECT["cur"] = "תנ'ך של לייה בח גלית: שיעור כפול רצוף (leah_do"
_ld=[]
for _d in range(6):
...
_SECT["cur"] = "ראשון ש5 וחמישי ש5 חייבים שיעור (h5_pin)"
...
_SECT["cur"] = "ח גלית: שישי מלא - כל 4 השעות עם מורה (galit_f"
for _h in range(1,5):
    m.Add(hfree[("ח גלית",(5,_h))]==0)

# מקצועות ליבה: לפחות שיעור כפול אחד בשבוע (שתי שעות רצופות)
_SECT["cur"] = "מקצועות ליבה: לפחות שיעור כפול אחד בשבוע (שתי "
...
_SECT["cur"] = "החטיבה מסיימת לא לפני שעה 5 (למעט שישי שהוא בן"
vend={}
for c in HCLASSES:
    for d in range(5):
...
_SECT["cur"] = "אין חלונות באמצע היום - חלון רק בסוף"
for c in HCLASSES:
    for d in range(6):
        for h in range(1,HDAY[d]):
...
_SECT["cur"] = "פיזור: לא יותר מ-2 שעות של אותו מקצוע ביום (חו"
for c in HCLASSES:
    for subj in NEED:
...
_SECT["cur"] = "עדיף לסיים מוקדם: קנס על שעות מאוחרות"
late=[]
for c in HCLASSES:
    for (d,h) in HSLOTS:
        if h>=6: late.append(hfree[(c,(d,h))].Not())
# עדיף מורה אחד למקצוע בכיתה - למזער פיצול
_SECT["cur"] = "עדיף מורה אחד למקצוע בכיתה - למזער פיצול"
split=[]
for c in HCLASSES:
    for subj in NEED:
...
_SECT["cur"] = "עדיף שהמחנך/ת ילמד/תלמד רב מלל בכיתה שלו/ה"
...
_SECT["cur"] = "עדיף מחנך מאותה שכבה (המקביל) על פני מורה משכב"
...
_SECT["cur"] = "חייב למלא בדיוק את מספר השעות -> hfree נקבע ע'"


# ================= קישור צולב: מורה אחד בכל רגע =================
_SECT["cur"] = "================= קישור צולב: מורה אחד בכל רגע"
...
_SECT["cur"] = "---- תקרה כוללת אמיתית (יסודי+חטיבה) למורים המ"
...
_SECT["cur"] = "---- NOGAP_CROSS: בלי חלונות למורים נבחרים, על"
for _t in ("תמיר",):
    for _d in range(6):
        _busy={}
        for _h in range(1,8):
...
_SECT["cur"] = "---- PARALLEL_EQ: כיתות מקבילות מסיימות באותה "
...
_SECT["cur"] = "---- WARM START: רמז מהפתרון הקודם -> פתרון מה"
_H={}
try:
...
_SECT["cur"] = "================= fills.json: תאי כיסוי (לצביע"
if st in (cp_model.OPTIMAL, cp_model.FEASIBLE):
