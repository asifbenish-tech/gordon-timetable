...
# הדרכת מתמטיקה עם שגית (יום ד) - עכשיו חוסמת
MATH_WED={"אינס":1,"דליה":2,"פנינה":3,"אנה":3,"צופיה":3,
          "יערה":4,"אביטל":4,"טלי":4,"תניה":5,"שרית":6}
for _t,_h in MATH_WED.items(): EVENTS.setdefault(_t,[]).append((3,_h))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_e.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_e.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_e.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
