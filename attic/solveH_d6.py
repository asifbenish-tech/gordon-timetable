...
m.Minimize(5000*sum(miss.values())+5*sum(late) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("d6.txt","w",encoding="utf-8").write(chr(10).join(
        f"MISS {c} {subj} = {sol.Value(v)}" for (c,subj),v in sorted(miss.items()) if sol.Value(v)) or "no subject misses")
