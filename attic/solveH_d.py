...
for c in HCLASSES:
    for d in range(5):
        for h in range(1,min(5,HDAY[d])+1):
            b=m.NewBoolVar(f"v{c}{d}{h}"); m.Add(free[(c,(d,h))]<=b); viol.append((c,d,h,b))
...
m.Minimize(3000*sum(b for _,_,_,b in viol) + 5*sum(late) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("viol.txt","w",encoding="utf-8").write(chr(10).join(
        f"{c} | {DAY_NAMES[d]} | שעה {h}" for c,d,h,b in viol if sol.Value(b)) or "אין")
