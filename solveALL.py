...
_v=[hx[("ט תמיר",s2,"חינוך","תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,"חינוך","תמיר") in hx]
if _v: m.Add(sum(_v)==2)
_v=[hx[("ט אסיף",s2,"אזרחות","תמיר")] for s2 in HSLOTS if ("ט אסיף",s2,"אזרחות","תמיר") in hx]
if _v: m.Add(sum(_v)==2)
_v=[hx[("ט אסיף",s2,"היסטוריה","תמיר")] for s2 in HSLOTS if ("ט אסיף",s2,"היסטוריה","תמיר") in hx]
if _v: m.Add(sum(_v)==2)
_v=[hx[("ט תמיר",s2,"היסטוריה","תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,"היסטוריה","תמיר") in hx]
if _v: m.Add(sum(_v)==2)
_v=[hx[("ט תמיר",s2,"אזרחות","תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,"אזרחות","תמיר") in hx]
if _v: m.Add(sum(_v)==2)
_v=[hx[("ט תמיר",s2,'תנ"ך',"תמיר")] for s2 in HSLOTS if ("ט תמיר",s2,'תנ"ך',"תמיר") in hx]
if _v: m.Add(sum(_v)==2)
for _kl in [k for k in hx if k[3]=="לייה" and k[1][1]>=6]:
    m.Add(hx[_kl]==0)
...
_v5m=[hx[k] for k in hx if k[0]=="ח גלית" and k[1]==(2,5) and k[2]=="מתמטיקה" and k[3]=="מורה חיצוני"]
...
for _kt6 in [k for k in hx if k[3]=="תמיר" and k[1] in ((4,6),(4,7)) and k[0]!="ז נעמי"]:
    m.Add(hx[_kt6]==0)   # חמישי 6-7: לא בז אלי (אלי שם) ולא בט (הלכו הביתה)
_va67=[hx[k] for k in hx if k[0]=="ז אלי" and k[1] in ((4,6),(4,7)) and k[3]=="אלי"]
...
if _a5t in hx: m.Add(hx[_a5t]==1)   # שלישי ש5: חינוך עם אסיף; אם חסר - בסוף היום
