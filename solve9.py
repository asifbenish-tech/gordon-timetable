...
EVENTS["סימה"]=[]          # + אלי (לא מלמד יסודי)
EVENTS["יערה"]=[(0,3)]; EVENTS["לייה"]=[(0,4)]
EVENTS["שרית"]=[(0,1)]; EVENTS["צופיה"]=[(0,2)]
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_g.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_g.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_g.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
