...
# ארז: 8 שעות מפוצלות שלישי/רביעי
...
# הדר: 4 שעות מתמטיקה בכל אחת משתי כיתות ז
for _c in [c for c in HCLASSES if GRADE[c]=="ז"]:
    _v=[x[(_c,s,"מתמטיקה","הדר")] for s in HSLOTS if (_c,s,"מתמטיקה","הדר") in x]
...
m.Minimize(9000*sum(miss.values())+400*sum(vend.values())+5*sum(late) - 80*sum(eli_own) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("dg.txt","w",encoding="utf-8").write(chr(10).join(
