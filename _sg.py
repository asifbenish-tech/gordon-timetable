...
m.Minimize(5000*sum(vgap.values())+400*sum(vend.values())+5*sum(late) - 80*sum(eli_own) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("gapv.txt","w",encoding="utf-8").write(chr(10).join(
