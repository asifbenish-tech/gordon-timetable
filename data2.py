...
# ימי חופש - מקור אחד לשני בתי הספר (יום חופשי הוא יום חופשי). hdata.HOFF נגזר מכאן.
DAYS_OFF2={
 ...
 "אסיף":["שישי"],
 "צבי":["ראשון","רביעי","חמישי"],   # שני-שלישי: משחקי חשיבה + הצטרפות להילה; שישי: ט אסיף (הנחה מהמערכת הנוכחית - לאמת עם צבי)
 "צופיה":[], "טלי":[], "שחר":["ראשון","חמישי","שישי"],
 'תל"ן':[],
 # מורי חטיבה בלבד
 "נעמי":["חמישי"], "אלי":["שישי"], "גלית":["שני"], "הדר":["שישי"], "הילה":["שישי"],
 "ארז":["ראשון","שני","שלישי","שישי"], "שיר":["ראשון","שני","שלישי","רביעי","חמישי"],
 "אופיר":["ראשון","שני","רביעי","חמישי","שישי"], "מדעים חיצוני":[],
}
# שעות חסומות נקודתית
UNAVAIL2={
...
# תיאור קצר "מה מלמד/ת" לכל מורה - לטבלת הצוות בלוח ולמסך האילוצים (טקסט חופשי,
# לא אילוץ; האילוצים עצמם ב-QUOTA/POOLS/CAP/TCONS)
TEACH_DESC={
 ...
 "צבי":"משחקי חשיבה ביסודי (5 שעות קבועות) · שישי בט אסיף · מצטרף לשיעורי הילה בשני-שלישי",
...
#   events_busy: True - גם ל-max_gap האירועים (HEV/EVENTS2) נחשבים תפוסים (ב-no_gaps תמיד)
TCONS = {
...
# מורים "צפים" ביסודי (בלי מכסה קבועה לכיתה): (תקרת שעות, אילו כיתות). כיתות: רשימה,
# "*" = כולן, או אותיות שכבות ("אבג"). המנוע (POOL) קורא מכאן.
ELEM_POOL = {"אופיר": (3, ["ו אורנה", "ו שרית"]), "יערה": (7, "*"), "דני": (0, "*"), "דניאל": (5, "*"),
             "אנה": (2, "*"), "אביטל": (4, "*"), "לייה": (4, "*"), "אינס": (0, "*"), "אורנה": (1, "*"),
             "טלי": (3, "ב"), "לי-אור": (6, "*"), "מרים": (15, "*"), "צופיה": (8, "אבג"), "שחר": (2, "אבג")}
# תקרה יומית בכיתה (המנוע: max(2, ceil(מכסה/ימים)) למורה עם מכסה קטנה) - חריגים כאן
DAYCAP_EXC = {("ד מירי", "לי-אור"): 3}
# אינס: זוג מדעים רצוף בכל אחת מהכיתות האלה (המנוע). הצעה שמוציאה אותה מכיתה - מסירה אותה מכאן.
INES_SCI_PAIR = ["ה דני", "ה תניה"]
...
# מורי FIXED_LESSONS (צבי): ביסודי רק השיעורים הקבועים - לא מועמדים למילוי חוסרים
# ולא נספרים כפנויים ביסודי בימים אחרים. זה לא "יום חופש": ימי החופש האמיתיים
# ב-DAYS_OFF2 למעלה (צבי עובד גם בחטיבה בשישי).
FIXED_ONLY = {t for (t, _l) in FIXED_LESSONS.values()}
FIXED_DAYS = {t: sorted({DAY_NAMES[d] for (_c, (d, _h)), (tt, _l) in FIXED_LESSONS.items() if tt == t}, key=DAY_NAMES.index) for t in FIXED_ONLY}
for _t in FIXED_ONLY:
