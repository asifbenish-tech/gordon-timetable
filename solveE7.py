...
blkh={}
cir_sun=m.NewBoolVar("cir_group2_on_sunday")   # קבוצה ב': ראשון או שלישי
SUN_ST={}
for tag,d in (("m",1),("u",2)):
    st=[m.NewBoolVar(f"{tag}st{k}") for k in (1,3,5)]      # בלוקים אמיתיים בלבד
    m.Add(sum(st)==1)
    if tag=="u": SUN_ST["st"]=st
    for h in range(1,DAY_HOURS[d]+1):
...
for c in CLASSES:
    for t in allowed[c]:
        for h in range(1,DAY_HOURS[1]+1):
            if t in CIR_MON and (c,(1,h),t) in x: m.Add(x[(c,(1,h),t)]+blkh[("m",h)]<=1)
        for h in range(1,DAY_HOURS[2]+1):
            if t in CIR_TUE and (c,(2,h),t) in x:
                m.Add(x[(c,(2,h),t)]+blkh[("u",h)]+cir_sun.Not()<=2)
            if t in NIHUL   and (c,(2,h),t) in x: m.Add(x[(c,(2,h),t)]+nb[h]<=1)
for h in range(1,DAY_HOURS[2]+1):
    m.Add(blkh[("u",h)]+nb[h]<=1)
for c in CLASSES:                                            # אם הקבוצה בראשון
    for t in allowed[c]:
        if t not in CIR_TUE: continue
        for h in range(1,DAY_HOURS[0]+1):
            if (c,(0,h),t) in x: m.Add(x[(c,(0,h),t)]+blkh[("u",h)]+cir_sun<=2)
...
m.Add(SUN_ST["st"][2]==0).OnlyEnforceIf(cir_sun)   # בראשון: לא 5-6 (מליאת צוות)
m.Minimize(10000*sum(empty.values())+sum(pen)+200*(len(farm)-sum(farm)))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("sed_new2.json","w",encoding="utf-8").write(json.dumps(
      {"מעגלי שיח שני":mb,"מעגלי שיח קבוצה ב":{"יום":("ראשון" if sol.Value(cir_sun) else "שלישי"),"שעות":ub},"ישיבת ניהול שלישי":nbh,
       "קבוצת שני":CIR_MON,"קבוצת שלישי":CIR_TUE},ensure_ascii=False,indent=1))
    ...
    io.open("sol_elem8.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    ...
    io.open("dev_elem8.txt","w",encoding="utf-8").write("\n".join(dev))
