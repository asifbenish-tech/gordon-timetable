...
for _t in ["אנה","פנינה","אביטל","דליה","דניאל","מירי","דני","אורנה","סימה"]:
    EVENTS[_t]=[e for e in EVENTS.get(_t,[]) if e[0]!=0]          # + אלי (לא מלמד יסודי)
EVENTS["יערה"]=[]; EVENTS["לייה"]=[]
EVENTS["שרית"]=[]; EVENTS["צופיה"]=[]
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    io.open("solution_f.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=1))
    io.open("circles_f.json","w",encoding="utf-8").write(json.dumps(
    ...
    io.open("deviations_f.txt","w",encoding="utf-8").write("\n".join(lines) or "אין סטיות")
