...
for c in HCLASSES:
    for subj in CORE:
        ...
        if ps:
            ...
            m.Add(sum(ps)+dv>=1); viol_dbl[(c,subj)]=dv
...
for c in HCLASSES:
    for d in range(5):
        ...
        for h in range(1,last+1):
            b=m.NewBoolVar(f"ve{c}{d}{h}"); m.Add(free[(c,(d,h))]<=b)
            viol_end[(c,d,h)]=b
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    for (c,subj),v in sorted(miss.items()):
        if sol.Value(v): lines.append(f"MISS SUBJ: {c} {subj} = {sol.Value(v)}")
    for (c,d,h),v in sorted(viol_end.items()):
        if sol.Value(v): lines.append(f"MISS END-TIME: {c} {DAY_NAMES[d]} {h}")
    for (c,subj),v in sorted(viol_dbl.items()):
        if sol.Value(v): lines.append(f"MISS DOUBLE: {c} {subj}")
    io.open("diag5.txt","w",encoding="utf-8").write(chr(10).join(lines))
