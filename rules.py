...
def r_sifrut_historia(c):
    ...
    for cz in ("ז נעמי", "ז אלי", "ח גלית"):
        for subj, t in (("ספרות", "נעמי"), ("היסטוריה", "אלי")):
            if cz == "ז אלי" and subj == "ספרות": continue   # ז אלי: שתי שעות הספרות אצל שיר בשישי (אלי 8 בכיתתו)
            v = [hx[(cz, s2, subj, t)] for s2 in HSLOTS if (cz, s2, subj, t) in hx]
            if v: m.Add(sum(v) >= 1)


def r_thursday_67(c):
    """חמישי 6-7: תמיר לא מלמד (חוץ מז נעמי); אלי חסום שם (HEV) - הדרישה שיהיה עם כיתתו בוטלה."""
    m, hx = c["m"], c["hx"]
...
RULES = [
    ...
    {"id": "tamir_subjects", "name": "המקצועות של תמיר בט", "active": True, "fn": r_tamir_subjects,
     "desc": 'חינוך, היסטוריה, אזרחות ותנ"ך בכיתות ט — שעתיים כל אחד (חינוך 3 בט אסיף; היסטוריה 1 בט אסיף), תמיר בלבד. שעת הגיבוש בשישי נספרת לו — 24 שעות.'},
    ...
    {"id": "sifrut_historia", "name": "ספרות והיסטוריה", "active": True, "fn": r_sifrut_historia,
     "desc": "נעמי (ספרות) ואלי (היסטוריה) — לפחות שעה אישית בכל כיתת ז ובח; בז אלי הספרות כולה אצל שיר בשישי."},
    {"id": "thursday_67", "name": "חמישי 6-7", "active": True, "fn": r_thursday_67,
     "desc": "תמיר ואלי לא מלמדים בשעות 6-7 של חמישי."},
