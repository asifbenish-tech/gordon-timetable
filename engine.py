...
m.Add(sum(v)<=13)                       # 13 שעות החלפה + 4 בשישי (א אנה) + 6 מקבילות = 23
                                        # 14 אינו אפשרי: נשארות לה 16 משבצות פנויות מחוץ
                                        # לשישי, ו-6 מהן תפוסות בשעות המקבילות
# צופיה: יום חופש חמישי
# שעות מקבילות של צופיה (מצטרפת לשיעור, לא מחליפה מורה).
# עם אנה - שעות קבועות לפי בקשה: שני ש5, שלישי ש1-ש2.
# עם פנינה - שתי שעות, המיקום לבחירת הפותר.
CO_FIX={"anna":[(1,5),(2,1),(2,2)]}
co={}
for tag,cls,hr,n in (("anna","א אנה","אנה",3),("pnina","א פנינה","פנינה",2)):
    vs=[]
    for sl in NONFRI:
