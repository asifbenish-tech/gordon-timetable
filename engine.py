...
# החלפה בשישי: תניה נכנסת לכיתה של דני ודני לכיתה שלה, באותה שעה בדיוק.
# כך היא מגיעה ל-6 שעות בכיתה של דני, בלי מורה שלישי שנכנס במקומם.
FRI_SWAP=3
fixed[("ה דני",(5,FRI_SWAP))]="תניה"     # תניה נכנסת לכיתה של דני
fixed[("ה תניה",(5,FRI_SWAP))]="דני"     # ודני לכיתה שלה - באותה שעה, בלי מורה שלישי
...
# צופיה: יום חופש קשיח בחמישי (בהתאם לבקשתה);
# שני ורביעי מתחילים ב-10:00 (משעה 3) - לפי בקשתה
for d in (4,):
    for h in range(1,DAY_HOURS[d]+1):
        for c in CLASSES:
...
# ---- תיקונים לפי המשוב ----
# דני: 11 בכיתתו + 9 אצל תניה = 20 בדיוק (כולל שעת ההחלפה בשישי).
# 10/10 אינו פתיר יחד עם ההחלפה בשישי - נבדק, וגם 12/8 ו-9/11 אינם פתירים.
...
m.Add(sum(v)==18-4-3)                   # משרה 18: 11 שעות החלפה + 4 בשישי (א אנה)
                                        # + 3 שעות מקבילות (2 עם אנה, 1 עם פנינה)
# צופיה: יום חופש חמישי
# שעות מקבילות של צופיה (מצטרפת לשיעור, לא מחליפה מורה).
# עם אנה - שעתיים קבועות: שלישי ש1-ש2.
# עם פנינה - שעה אחת, המיקום לבחירת הפותר.
CO_FIX={"anna":[(2,1),(2,2)]}          # ירדה השעה של שני ש5
co={}
for tag,cls,hr,n in (("anna","א אנה","אנה",2),("pnina","א פנינה","פנינה",1)):
    vs=[]
    for sl in NONFRI:
        b=m.NewBoolVar(f"co{tag}{sl}")
        if tag in CO_FIX:
            m.Add(b==(1 if sl in CO_FIX[tag] else 0))
        if sl[0]==4 or (sl[0] in (1,3) and sl[1] in (1,2)) or sl in BLOCK["צופיה"]:
            # גם השעות המקבילות כפופות לאילוצים שלה: חמישי חופש, שני/רביעי
            # מתחילים בשעה 3, והדרכת שפה בראשון ש2
            m.Add(b==0); vs.append(b); co[(tag,sl)]=b; continue
        if sl[0]==2 and sl[1] in nb:              # לא בזמן ישיבת מרכזי בית חינוך
            m.Add(b+nb[sl[1]]<=1)
        k=(cls,sl,hr)
...
for c in CLASSES:
    for t in allowed[c]:
        ...
        # התקרה היומית נועדה למורה עם מכסה קטנה שלא ייערם ביום אחד - לא
        # למחנך/ת בכיתתו/ה. אינס נתפסה בה: 12 פחות 4 שעות שישי קבועות = 8,
        # ושעות חצי-הכיתה נספרות לה, ולכן אחרי בוקר של חצי-כיתה היא לא
        # יכלה להישאר עם הכיתה. הפטור למחנכים לא מזיז אף תא (נבדק).
...
for _c8 in CLASSES:
    for _t8 in allowed[_c8]:
        ...
        # חריג נקודתי: לי-אור בד מירי בחמישי ש1-ש3 (בקשת המנהל 05.09).
        # ההחלפה מול אינס בש3 היא ישירה, ולכן הבלוק שלה נעשה בן 3 שעות.
        if (_c8,_t8)==("ד מירי","לי-אור"): continue
        for _d8 in range(5):
...
for _cf,_df in list(TLN_FIXED_DAY.items())+[(c,None) for c in TLN_FULL_CONSEC]:
    ...
    for _kf in [k for k in hf if k[0]==_cf]: m.Add(hf[_kf]==0)   # בלי חצי כיתה
    _adj=[]
    for _dd in ([_df] if _df is not None else range(5)):
        for _hf2 in range(1,DAY_HOURS[_dd]):
            _a2,_b3=(_cf,(_dd,_hf2),'תל"ן'),(_cf,(_dd,_hf2+1),'תל"ן')
...
# ד אינס: שתי שעות החצי-כיתה של כל מורת תל"ן ברצף. היום נבחר ע"י הפותר -
# קיבוע חגית ליום שני פתיר אבל מוסיף חוסר רביעי, ולכן ויתרנו עליו.
for _u,_days in (("יעל",range(5)),("חגית",range(5))):
    _iad=[]
    for _di in _days:
        for _hi in range(1,DAY_HOURS[_di]):
            _k1,_k2=("ד אינס",(_di,_hi),_u),("ד אינס",(_di,_hi+1),_u)
            if _k1 in hf and _k2 in hf:
                _pi=m.NewBoolVar(f"iad{_u}{_di}{_hi}")
                m.Add(hf[_k1]+hf[_k2]==2).OnlyEnforceIf(_pi); _iad.append(_pi)
    if _iad: m.Add(sum(_iad)>=1)

# ד אינס: בכל שעה חצויה, החצי שנשאר בכיתה הוא עם אינס עצמה - לא עם מורה
# אחר/ת. כך שתי שעות החצי-כיתה של יעל (וגם של חגית) הן שעות של המחנכת.
for _kh in [k for k in hf if k[0]=="ד אינס"]:
    _kr=("ד אינס",_kh[1],"אינס")
...
# שני ש2: דניאל עובר לה דני במקום אנה, ומרים נכנסת במקומו לג דניאל
# (בקשת המנהל 03.09; מרים עלתה ל-25 שעות פרונטליות).
if ("ה דני",(1,2),"דניאל") in x: m.Add(x[("ה דני",(1,2),"דניאל")]==1)
if ("ג דניאל",(1,2),"מרים") in x: m.Add(x[("ג דניאל",(1,2),"מרים")]==1)

# חמישי ש3: החלפה ישירה בין לי-אור לאינס (בקשת המנהל 05.09) - לי-אור
# עוברת מו שרית לד מירי, ואינס בכיוון ההפוך. שתי השעות האחרות של
# לי-אור בד מירי באותו יום נשארות במקומן, ולכן ההחלפה נוגעת בשתי
# מערכות בלבד ולא גוררת מורה שלישי/ת.
if ("ד מירי",(4,3),"לי-אור") in x: m.Add(x[("ד מירי",(4,3),"לי-אור")]==1)
if ("ו שרית",(4,3),"אינס") in x: m.Add(x[("ו שרית",(4,3),"אינס")]==1)
for _hL in (1,2):
    if ("ד מירי",(4,_hL),"לי-אור") in x: m.Add(x[("ד מירי",(4,_hL),"לי-אור")]==1)

# ראשון ש4: החלפה ישירה - אינס חוזרת לכיתתה, מרים נכנסת לו שרית במקומה
# (בקשת המנהל 16.09; אינס ולי-אור לא נכנסות לו שרית).
if ("ד אינס",(0,4),"אינס") in x: m.Add(x[("ד אינס",(0,4),"אינס")]==1)
if ("ו שרית",(0,4),"מרים") in x: m.Add(x[("ו שרית",(0,4),"מרים")]==1)

# שלישי ש1: מירי נכנסת לד אינס במקום אביטל (בקשת המנהל 03.09);
# את השעה שהיא עוזבת בכיתתה לוקח צבי - ראה FIXED_LESSONS.
if ("ד אינס",(2,1),"מירי") in x: m.Add(x[("ד אינס",(2,1),"מירי")]==1)
...
_QUOTA={"אלי":20,"גלית":20,"נעמי":20,"צבי":12,"רובי":6}          # חטיבה בלבד
_QUOTA.update({_t:MAXQ2[_t] for _t in ("תמיר","מרים","לייה","אסיף","שרית","חסן")})  # MAXQ2 = מקור אחד
