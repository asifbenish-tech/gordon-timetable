...
for c in HCLASSES:
    ...
    for subj,per in NEED.items():
        ...
        ov=m.NewIntVar(0,3,f"ov{c}{subj}"); over[(c,subj)]=ov
        m.Add(sum(v)+sh-ov==per[g])
...
m.Minimize(8000*sum(miss.values())+8000*sum(over.values())+5*sum(late) - 150*sum(fri) - 50*sum(hadar) - 200*sum(own) - 120*sum(same) + 60*sum(split))
...
if st in (cp_model.OPTIMAL,cp_model.FEASIBLE):
    ...
    L=[f"MISS {c}/{sj} -{sol.Value(v)}" for (c,sj),v in sorted(miss.items()) if sol.Value(v)]
    L+=[f"OVER {c}/{sj} +{sol.Value(v)}" for (c,sj),v in sorted(over.items()) if sol.Value(v)]
    io.open("d8.txt","w",encoding="utf-8").write(chr(10).join(L) or "none")
